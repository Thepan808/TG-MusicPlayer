...
async def playfrom(client, m: Message):
 if GRPPLAY or (m.from_user and m.from_user.is_contact) or m.outgoing:
   chat_id = m.chat.id
   if len(m.command) < 2:
      await m.reply("**USE:** \n\n`/playfrom [chat_id/username]` \n`/playfrom [chat_id/username] ; [play de música]`")
   else:
      ...
      if ";" in args:
         ...
         chat = args
         limit = 10
      hmm = await m.reply(f"Procurando o último **{limit}** de Músicas para `{chat}`")
      try:
