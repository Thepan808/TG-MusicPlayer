   ...
   if replied:
      if replied.video or replied.document:
         ...
         if chat_id in QUEUE:
            ...
            await huehue.edit(f"Em fileira em posição **#{pos}**")
         else:
            if Q==720:
            ...
            await huehue.edit(f"**Iniciou a reprodução de Video ▶** \n**♦️ Vídeo** : [{songname}]({link}) \n**♦️ NO CHAT** : `{chat_id}`", disable_web_page_preview=True)
      else:
         if len(m.command) < 2:
            await m.reply("`Responda a um arquivo de vídeo ou dê algo para pesquisar`")
         else:
         ...
         if len(m.command) < 2:
            ...
            if search==0:
               ...
               if hm==0:
                  ...
                  if chat_id in QUEUE:
                     pos = add_to_queue(chat_id, songname, ytlink, url, "Video", Q)
                     await huehue.edit(f"Em fileira na posição **#{pos}**")
                  else:
                     try:
                        await call_py.join_group_call(
                           chat_id,
...
async def vstream(client, m: Message):
   chat_id = m.chat.id
   if len(m.command) < 2:
      await m.reply("`Dê um link/LiveLink/.m3u8 URL/YTLink para transmitir ♦️`")
   else:
      if len(m.command)==2:
         link = m.text.split(None, 1)[1]
         Q = 720
         huehue = await m.reply("`Tentando transmitir 💭`")
      ...
      if hm==0:
         ...
         if chat_id in QUEUE:
            pos = add_to_queue(chat_id, "Live Iniciada 📺", livelink, link, "Video", Q)
            await huehue.edit(f"Na fila em posição **#{pos}**")
         else:
            if Q==720:
               hmmm = HighQualityVideo()
            elif Q==480:
            ...
            try:
               ...
               add_to_queue(chat_id, "Live em reprodução 📺", livelink, link, "Video", Q)
               await huehue.edit(f"Iniciou **[Live Online pae 📺]({link})** em `{chat_id}`", disable_web_page_preview=True)
            except Exception as ep:
               await huehue.edit(f"`{ep}`")
