   ...
   if replied:
      if replied.video or replied.document:
         ...
         if chat_id in QUEUE:
            ...
            await huehue.edit(f"Em posição **#{pos}**")
         else:
            if Q==720:
               ...
               if hm==0:
                  ...
                  if chat_id in QUEUE:
                     ...
                     try:
                        ...
                        await huehue.edit(f"**Transmissão de Video iniciado ▶** \n**♦️ Vídeo** : [{songname}]({url}) \n**♦️ Chat** : `{chat_id}`", disable_web_page_preview=True)
...
async def vstream(client, m: Message):
 if GRPPLAY or (m.from_user and m.from_user.is_contact) or m.outgoing:
   chat_id = m.chat.id
   if len(m.command) < 2:
      await m.reply("`Dê um link/LiveLink/.m3u8 URL/YTLink para transmitir 🎶`")
   else:
      if len(m.command)==2:
         ...
         if quality == "720" or "480" or "360":
            ...
            Q = 720
            await m.reply("`Somente 720, 480, 360 Permitido` \n`Agora transmitir em 720p`")
         huehue = await m.reply("`Tentando transmitir 💭`")
      else:
      ...
      if hm==0:
         ...
         if chat_id in QUEUE:
            ...
            try:
               ...
               await huehue.edit(f"Iniciado sua vossa **[Live Stream de gay 📺]({link})** em `{chat_id}`", disable_web_page_preview=True)
            except Exception as ep:
               await huehue.edit(f"`{ep}`")
